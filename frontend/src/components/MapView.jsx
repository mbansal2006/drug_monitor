...
const MapView = ({ locations }) => {
  ...
  useEffect(() => {
    ...
    map.eachLayer(l => {
      if (l.id.startsWith('loc-') || l.id === 'heat') {
    ...
    if (mode === 'markers') {
      locations.forEach((loc) => {
        ...
        new mapboxgl.Marker(el)
          ...
          .setPopup(new mapboxgl.Popup({ offset: 25 }).setHTML(`<div class='text-sm'><strong>${loc.name}</strong><br/>Risk: ${loc.risk_score}</div>`))
          .addTo(map);
      });
    } else {
      const geojson = {
      ...
      if (!map.getSource('heat')) {
        ...
        map.getSource('heat').setData(geojson);
      }
    }
  }, [locations, mode]);
