...
df = pd.read_csv("ndcs.csv")
os.makedirs("spl_xml_files", exist_ok=True)
...
for i, row in df.iterrows():
    ...
    if ndc_code in seen:
        ...
        if not spl_set_id:
            ...
            if not xml_text:
                ...
                file_path = f"spl_xml_files/{ndc_id}.xml"
    ...
    if i % 50 == 0:
        print(f"Processed {i} NDCs")

    sleep(0.25)

pd.DataFrame(log_rows).to_csv("spl_xml_log.csv", index=False)