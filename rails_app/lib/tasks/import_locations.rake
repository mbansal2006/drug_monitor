namespace :import do
  ...
  task locations: :environment do
    require 'csv'
    Location.skip_callback(:validation, :after, :geocode, if: ->(obj) { obj.latitude.blank? || obj.longitude.blank? })
    path = Rails.root.join('public', 'site_csvs', 'site_locations_table.csv')
    puts "Loading #{path}"
    CSV.foreach(path, headers: true, header_converters: :symbol) do |row|
      begin
        attrs = row.to_h.slice(
          :address, :country, :state_or_region, :postal_code,
        ...
        attrs[:csv_location_id] = row[:location_id].to_i if row[:location_id]
        ...
        puts "Imported location_id #{attrs[:csv_location_id]}"
      rescue => e
        puts "Error processing row: #{e.message}"
      end
    end
    Location.set_callback(:validation, :after, :geocode, if: ->(obj) { obj.latitude.blank? || obj.longitude.blank? })
  end
end